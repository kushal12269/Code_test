...
export class MomentDateTimeAdapter extends DateTimeAdapter<Moment> {
    ...
    public createDate( year: number, month: number, date: number, hours: number = 0, minutes: number = 0, seconds: number = 0 ): Moment {
        ...
        if (!this.checkHoursMinutesExists(hours, minutes, seconds) && (window as any).projectStartTime) {
            hours = (window as any).projectStartTime.hours;
            minutes = (window as any).projectStartTime.minutes;
        }
        seconds = 0;
    ...
    /**
     * Function to check if hours,minutes or seconds exists. This is custom function.
     * @param hours - Hours in integer format
     * @param minutes - Minutes in integer format
     * @param seconds - Seconds in integer format
     * @returns If hours, minutes and seconds exists.
     */
    public checkHoursMinutesExists(hours, minutes, seconds) {
    ...
    public now(): Moment {
        //Modification to set the project start time as default time.
        if ((window as any).projectStartTime) {

            const projectDate = new Date().setHours((window as any).projectStartTime.hours, (window as any).projectStartTime.minutes, (window as any).projectStartTime.seconds);
            return this.createMoment(projectDate).locale(this.locale); 
        }
